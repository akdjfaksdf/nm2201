...
var startcheck = function() {
	...
	if (p1ready == 0 && p2ready == 1) {
		console.log(`waiting for p1`);
		p2waitalert.innerText = 'waiting for player 1';
	}

	if (p1ready == 1 && p2ready == 0) {
...
var drawMaze = function() {
	...
	while (visitedCount < rows*cols) {
		...
		if (next) {
			...
			current = stack.pop();
			console.log(`next is undefined, backtracking now; in ${stack.pop()}`);
		};
	}
...
var drawCell = function (x, y) {
	...
	celldrawn.walls = [];
		...
		var cellx = celldrawn.x*cellw;
		var celly = celldrawn.y*cellh;

		//up wall
		celldrawn.walls[0] = paper.path(`M ${cellx} ${celly} L ${cellx + cellw} ${celly}`).attr({'stroke-width': 3});
		
		//down wall
		celldrawn.walls[1] = paper.path(`M ${cellx} ${celly + cellh} L ${cellx + cellw} ${celly + cellh}`).attr({'stroke-width': 3});
		//left wall
		celldrawn.walls[2] = paper.path(`M ${cellx} ${celly} L ${cellx} ${celly + cellh}`).attr({'stroke-width': 3});
		//right wall
		celldrawn.walls[3] = paper.path(`M ${cellx + cellw} ${celly} L ${cellx + cellw} ${celly + cellh}`).attr({'stroke-width': 3});
	...
	celldrawn.visitedRect = paper.rect(cellx, celly, cellw, cellh).attr({'fill': 'yellow',
		'stroke-width': 0 	
	});
