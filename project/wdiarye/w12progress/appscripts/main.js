...
var p1char1, p1char2, p1char3;
var p2char1, p2char2, p2char3;

//finishing lines
var lastCol = [];
var firstCol = [];

var keydownfunc = true;
...
var drawCharacters = function() {
	...
	p1char1 = (p1charGridSet1[Math.floor(Math.random()*rows/3)]);
	p1char2 = (p1charGridSet2[Math.floor(Math.random()*rows/3)]);
	p1char3 = (p1charGridSet3[Math.floor(Math.random()*rows/3)]);

	p1char1.visitedRect.attr({'fill': 'red'}); 
	p1char2.visitedRect.attr({'fill': 'red'});
	p1char3.visitedRect.attr({'fill': 'red'});

	p2char1 = (p2charGridSet1[Math.floor(Math.random()*rows/3)]);
	p2char2 = (p2charGridSet2[Math.floor(Math.random()*rows/3)]);
	p2char3 = (p2charGridSet3[Math.floor(Math.random()*rows/3)]);

	p2char1.visitedRect.attr({'fill': 'white'}); 
	p2char2.visitedRect.attr({'fill': 'white'});
	p2char3.visitedRect.attr({'fill': 'white'}); 
...
var moveChar = function(ev) {
	console.log(`in keydown func, ${ev.key}`);


	//if p1char1 is at end, p1char2 can go, then p1char3


	//if p1char is at end, then turn p1char blue (and animate character into p2div?)
	var checkp1fin = function() {
	console.log('in checkp1fin func');
	
		for (i = 0; i < lastCol.length; i++) {
			if (p1char1.xy === lastCol[i].xy) {
				p1char1.visitedRect.attr({'fill': 'blue'});
				keydownfunc = false; 
			};
		};
	...
	if (keydownfunc === true) {
		...
		switch(ev.key){
			...
			//up
			case 'w':
				if (p1char1.walls[0].attrs['stroke-width'] === 0) {

					//if previous rect was a char, change it back to p1char (red) or p2char (white)
					if (p1char1.xy === p1char2.xy || p1char1.xy === p1char3.xy){
						p1char1.visitedRect.attr({'fill': 'red'});
					}
					else if (p1char1.xy === p2char1.xy || p1char1.xy === p2char2.xy || p1char1.xy === p2char3.xy) {
						p1char1.visitedRect.attr({'fill': 'white'});
					}
					//if not, change previous rect back to purple
					else {
					p1char1.visitedRect.attr({'fill': 'purple'});
					};

					//abandoned animation idea
					// var anim = Raphael.animation({x: p1char1.cellx, y: p1char1.celly-cellw}, 100);
					// p1char1.visitedRect.animate(anim);

					p1char1 = grid[index(p1char1.x, p1char1.y-1)];
					p1char1.visitedRect.attr({'fill': 'red'});
					checkp1fin();
			...
			//down
			case 's': 
				if (p1char1.walls[1].attrs['stroke-width'] === 0) {
			...
			//left
			case 'a': 
				if (p1char1.walls[2].attrs['stroke-width'] === 0) {
					...
					p1char1 = grid[index(p1char1.x-1, p1char1.y)];
					p1char1.visitedRect.attr({'fill': 'red'});
					checkp1fin();
			...
			//right
			case 'd': 
				if (p1char1.walls[3].attrs['stroke-width'] === 0) {
